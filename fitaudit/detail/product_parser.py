...
def get_quantity(s: Tag) -> (float, str):
    while s.name != 'span':
        s = s.next
        continue
    t = s.text.replace(',', '.')
...
def product_info_parser(soup):
    ...
    content.update({
        ...
        'f': get_quantity(him.find_next('span', class_=S.f_span).next),
        'p': get_quantity(him.find_next('span', class_=S.p_span).next),
        'c': get_quantity(him.find_next('span', class_=S.c_span).next),
        'w': get_quantity(him.find_next('span', class_=S.w_span).next),
        'a': get_quantity(him.find_next('span', class_=S.a_span).next),
    })

    him_summary, him_bju_daily, him_vitamins, him_minerals = him.find_next_siblings('p')

    # summary
    product.update({'summary': dict()})
